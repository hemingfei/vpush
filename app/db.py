...
class DB:
    ...
    def _news_article_filter(
        ...
        if source_id is not None:
            ...
            params.append(user_id)
            # 周刊只在按期书架里读，不混进「全部资讯」时间线。
            conds.append("COALESCE(s.kind, 'feed') != 'magazine'")
        if unread:
    ...
    def unread_news_count(self, user_id: int) -> int:
        ...
        rows = self._rows(
            ...
            "AND COALESCE(s.kind, 'feed') != 'magazine' "
    ...
    def unread_news_counts_by_source(self, user_id: int) -> dict[int, int]:
        rows = self._rows(
            ...
            "WHERE s.archived_at IS NULL AND s.enabled = 1 "
            "AND COALESCE(s.kind, 'feed') != 'magazine' AND " + self._NEWS_UNREAD_SQL +
