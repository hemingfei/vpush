...
function turnstileSettingsHtml(info) {
  ...
  return `${warn}<section class="section-panel" id="ts-form" data-secret-set="${info.secret_set ? "1" : "0"}">
    ...
    <div class="toolbar">
      <button type="button" class="btn-normal btn-fluid" id="ts-save" onclick="saveTurnstileSettings()">保存登录验证</button>
      <button type="button" class="btn-ghost btn-fluid" onclick="pasteCookieField('ts-secret').then(markTurnstileDirty)">从剪贴板填入密钥</button>
    </div>
  </section>`;
}
