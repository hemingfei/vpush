...
- 顶层导航在“动态”之后增加独立“财经资讯”（手机底部导航显示“资讯”），页内分「实时资讯」「财经新闻」两个页签，默认展示实时资讯；不与动态快讯共用页签。
- 实时资讯是管理员在后台勾选的大V消息动态流：帖子卡片样式与动态页一致（头像/平台标/时间/正文/配图/原文链接），无限滚动加载，60 秒增量轮询自动置顶新帖。
- 列表按发布时间倒序混排，显示媒体、时间、新标记、标题、两行摘要和可用首图；来源选择使用带搜索的复选对话框，临时筛选使用单一原生下拉。
- 全文页显示媒体、标题、作者、时间、清洗后的正文和原文链接。正文图片通过登录后的图片 API 获取 Blob，不保留第三方 `src`。
- 财经新闻首版仅限登录后的自托管阅读，不公开、不索引、不站外分享、不全文推送。
