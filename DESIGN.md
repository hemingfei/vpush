...
- 顶层导航在“动态”之后增加独立“财经资讯”（手机底部导航显示“财经新闻”），页内分「实时资讯」「调研纪要」「财经新闻」三个页签，默认展示实时资讯；不与动态快讯共用页签。
- 实时资讯/调研纪要是管理员在后台勾选的大V消息动态流：帖子卡片样式与动态页一致（时间/大V名/正文/配图/原文链接），无限滚动加载，60 秒增量轮询——新帖攒进待读缓冲，用户在顶部时自动合并，在下方时弹胶囊提示，点击或滚回顶部再合并；带搜索与来源下拉。
- 财经新闻列表为全部未归档源全局混排（不按用户订阅圈过滤，无「我的来源」）；按发布时间倒序、以「今天 / 昨天 / M/D」日分隔线分组；文案先行（标题两行 + 摘要两行），首图缩至右侧 84px 小图（进入视口再加载）；工具条含「只看未读」实底开关（带未读计数）、来源下拉与搜索，未读多时以「全部已读」收尾。
- 未读以 `news_last_seen_at` 水位线为口径（浏览列表即推进，文章详情页不推进）；侧栏与底栏入口带未读数徽标（Duty Blue 胶囊，>99 显示 99+）。
- 全文走弹窗（非整页路由）：弹窗内显示媒体、标题、作者、时间、工具行（原文链接 + 小/标准/大三档字号切换，记忆到账号）和清洗后的正文；正文列宽上限 700px；底部上一篇 / 下一篇成对导航。正文图片通过登录后的图片 API 获取 Blob（内存缓存），不保留第三方 `src`。
- 财经新闻仅限登录后的自托管阅读，不公开、不索引、不站外分享、不全文推送。
