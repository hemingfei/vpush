...
| healthcheck | 目录 + sqlite + 列 `/vpush` | **已入库** `scripts/healthcheck.py`（写 `manifest/` 与 `logs/health.json`；ops 读这份，不再依赖 compose healthcheck） |
...
| arm-lab-ops | 实验室看板 + 115 QR + 确认后的限量动作 + 同步/并发旋钮 | **宿主机 systemd** `vpush-arm-lab-ops.service`（Tailscale `:8055`；备选 loopback + SSH；不是生产后台；**不要**写进生产 compose，也不要再跑 ops 容器） |
...
8. ARM 实验室运维面板 — **宿主机 systemd** `vpush-arm-lab-ops.service`（状态 + 115 QR + 确认后的 IMA/CICC 限量触发与 failed 重入 + 同步时钟/并发旋钮；不是阅读台 / 生产后台）
...
- **访问（口令仍要）：** Oracle-SJ-ARM 走 Tailscale。① 优先绑 Tailscale IPv4（`ARM_OPS_BIND=tailscale` 或 `tailscale ip -4` 的地址）`:8055`，同 tailnet 打开该 URL。② 备选 `127.0.0.1:8055` + `ssh -L 8055:127.0.0.1:8055`。③ **不要**在公网 NIC 发布 `0.0.0.0:8055`。uvicorn 绑 IP 不绑网卡名；**首选** `vpush-arm-lab-ops.service`。`docker-compose.snippet.yml` 只是遗留 fallback。也可用 `tailscale serve` 挂在 loopback 前面。
- **口令：** `ARM_OPS_PASSWORD` 或 `ARM_OPS_PASSWORD_FILE`（宿主机 `/opt/vpush-ima-lab/secrets/arm-ops-password.txt`）。Session Cookie `arm_ops`（HttpOnly, SameSite=Lax）。
- **115 QR：** 本面板拥有扫码写宿主机 `secrets/115-cookies.txt`（0600）的路径；JSON **只回** `{ok, cookie_len}`，不回 Cookie 正文。设备默认 `harmony`，与 p115client apps 一致。缺 `p115client` 则扫码失败并保持关闭。
...
- **路径：** phase 2 需要 `CACHE_ROOT` **rw**（requeue / audit）。宿主机 unit 直接读 `/opt/vpush-ima-lab/src` 与 `/opt/vpush-ima-lab/venv`（`VPUSH_PYTHON`）。凭据只在宿主机 `secrets/`：`115-cookies.txt`、`cicc-cookies.txt`、`ima-pure.json`、`arm-ops-password.txt`（**不要提交**）。样本 unit 把这些写成 host 路径，不再映射 `/secrets`，也不挂 `docker.sock`。
...
本地跑法与宿主机 unit 安装见 [arm-lab-ops/README.md](../arm-lab-ops/README.md)。样本 unit 见 [arm-lab-ops/systemd/](../arm-lab-ops/systemd/)。`docker-compose.snippet.yml` 只是遗留 fallback。
