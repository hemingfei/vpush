...
export function createMarketView({ api, escapeHtml }) {
  ...
  function sparkline(item) {
    ...
    if (history.length < 2) return '<span class="market-spark-empty" aria-label="暂无走势">--</span>';
    const values = history.map(point => point.close);
    const low = Math.min(...values);
    const high = Math.max(...values);
    const y = value => high === low ? 18 : 32 - (value - low) / (high - low) * 28;
    const points = values.map((value, i) => `${(2 + i / (values.length - 1) * 60).toFixed(1)},${y(value).toFixed(1)}`).join(" ");
    const change = (values.at(-1) / values[0] - 1) * 100;
    const label = `${item.name}，${history[0].date} 至 ${history.at(-1).date}，${signed(change)}%${item.history_stale ? "，走势更新失败" : ""}`;
    return `<svg class="market-spark ${tone(change)}" viewBox="0 0 64 36" role="img" aria-label="${escapeHtml(label)}">
      <title>${escapeHtml(label)}</title>
      <line class="market-spark-baseline" x1="2" x2="62" y1="${y(values[0])}" y2="${y(values[0])}" />
    ...
    function render() {
      ...
      host.innerHTML = `<div class="market-heading">
        ...
        <table class="market-table" aria-labelledby="market-title">
          <thead class="sr-only"><tr><th scope="col">标的</th><th scope="col">近20个交易日走势</th><th scope="col">点位与涨跌幅</th></tr></thead>
          <tbody>${rows}</tbody>
        </table>
        <div class="market-footer"><span>近20交易日</span>
