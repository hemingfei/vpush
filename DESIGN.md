...
- 顶层导航在“动态”之后增加独立“财经新闻”，直接进入个人来源新闻列表；不与动态快讯共用页签。侧栏与移动底栏入口带未读数徽标（Duty Blue 胶囊，>99 显示 99+），read-all 与浏览后即时同步。
- 列表按发布时间倒序、以「今天 / 昨天 / M/D」日分隔线分组；条目文案先行（标题两行 + 摘要两行），首图缩至右侧 84px 小图；工具条一行内放「只看未读」实底开关（带未读计数）、来源下拉与搜索，来源多时以「全部已读」收尾。
- 未读以 `news_last_seen_at` 水位线为口径；「只看未读」过滤下空态给正反馈文案（「没有未读文章」），且不推进水位（避免看一篇少一篇的误清）。
- 来源选择使用带搜索的复选对话框，按管理端分组分节展示（未分组置底）；列表筛选下拉同分组用 optgroup。
- 全文页显示媒体、标题、作者、时间、工具行（原文链接 + 小/标准/大三档字号切换，记忆到账号）和清洗后的正文；正文列宽上限 700px，顶部 2px Duty Blue 阅读进度条（尊重 reduced-motion）；底部上一篇 / 下一篇成对导航。正文图片通过登录后的图片 API 获取 Blob（ETag 缓存），不保留第三方 `src`。
