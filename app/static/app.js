  ...
  return `
    <div class="post-item" data-post-id="${post.id}">
      ...
      <div class="p-meta">
        ...
        <button type="button" class="p-action cat-export post-card-export" onclick="exportPostCard(${post.id}, event)" aria-label="复制图卡" title="复制图卡">${IMAGE_CARD_ICON} 图卡</button>
        ${post.platform === "zsxq" ? "" : `<a class="p-action" href="${escapeHtml(safeUrl)}" target="_blank" rel="noopener" aria-label="查看原文" title="查看原文">查看原文 ${EXTERNAL_LINK_ICON}</a>`}
        </div>
      </div>
    </div>`;
}
