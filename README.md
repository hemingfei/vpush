...
- **抓取策略**：优先大V短间隔（默认 60s）+ 普通大V全局间隔（默认 180s），随机抖动与失败退避；**无新帖自动降频**（空轮间隔 2 倍步进，优先大V封顶 180s、普通大V封顶 900s，有新帖立即恢复），轮内随机错峰打破固定节律，X 降级 RSSHub 期间限速；微博 cookie 自动续期保活（登录失败 30 分钟冷却），雪球 cookie 定时探测失效并告警（雪球需手动更新 Cookie，需手动更新 cookie）
