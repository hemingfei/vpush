...
- 每行一个 JSON：`{"id","title","summary","tags":[...],"day","publish","authors"}`，`id` 为字符串报告 id
...
- 发布日期：`publish`（`YYYY-MM-DD`，来自列表 publishTime 的北京日期）写入记录 `pub_date`，作为读模型跨年排序键 `sort_date` 的来源；非法或缺失留空（排序沉底），`day` 的 MMDD 语义不变
