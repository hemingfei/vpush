...
- 管理员标注立即生效；多位现任管理员标注不一致时，最近更新者优先；
- 授权用户（后台白名单）按「一致人数」生效：同一消息上标注出相同
  「股票+操作」的人数达到 agree_n（默认 2）即生效；同时有两个及以上不同
  标注组达标（分歧）则不生效，等管理员定夺；
...
def resolve_effective_marks(marks, config: dict) -> dict[int, dict]:
    ...
    管理员独裁优先（最新的现任管理员标注生效）；否则按 (target, action) 分组，
    恰好一个组达到 agree_n 人生效，0 个或多个（分歧）不生效。
    """
    ...
    for post_id, rows in by_post.items():
        ...
        if admin_marks:
            ...
            effective[post_id] = {
                "target_name": str(best["target_name"]),
        ...
        for m in valid:
            groups.setdefault((str(m["target_name"]), str(m["action"])), []).append(m)
        reached = [(key, voters) for key, voters in groups.items() if len(voters) >= agree_n]
        if len(reached) == 1:
            (name, action), voters = reached[0]
            effective[post_id] = {
                "target_name": name,
                "action": action,
                "by_admin": False,
...
def effective_marks_for_kol(db, kol_id, since_day: str, config: dict | None = None) -> dict[int, dict]:
...
def marks_summary_for_posts(db, post_ids, viewer: dict | None,
    ...
    返回 {post_id: {marks: [...], effective: {...}|None, my_mark: {...}|None}}；
    marks 按时间升序含标注人身份，effective 为空表示标注未生效（未达标或分歧）。
    """
    cfg = config or get_mark_config(db)
    ...
    for post_id in ids:
        ...
        mine = next((m for m in marks if int(m["user_id"]) == viewer_id), None)
        out[post_id] = {
            "marks": [
                {
            ...
            "my_mark": ({"target_name": str(mine["target_name"]), "action": str(mine["action"])}
                        if mine else None),
        }
    return out
...
def attach_mx_action_marks(db, posts, viewer: dict | None, config: dict | None = None) -> list:
    ...
    卡片角标数据源：已生效出「人工:操作」实心角标；有标注未生效出
