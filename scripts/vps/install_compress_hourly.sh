#!/bin/bash
# IMA PDF v3 增量压缩 timer：每小时低优先级处理新文件和内容被覆写的旧路径。
# 从仓库运行本脚本会同步部署 compressor、collector 和使用同一目录锁的 puller。
# 回滚：使用 /root/cicc/backup-compress-v2-* 恢复脚本和 unit。
set -euo pipefail
...
/usr/bin/python3 -m py_compile /root/cicc/cicc_report_collector.py \
  /root/cicc/pdf_backfill_compress.py /usr/local/lib/vpush-ima/ima-puller.py
...
systemctl enable --now vpush-compress-hourly.timer
echo "v3 compressor, puller lock, and hourly timer installed"
