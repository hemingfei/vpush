...
# style.css 之外的补充样式表：一并纳入内容摘要、哈希 URL 与 index.html/sw.js 引用维护，
# 改动它们也会推进 CACHE 版本，避免老客户端拿旧样式配新后端
EXTRA_STYLESHEETS = ("mx-views.css", "holdings.css", "mx-kol-holdings.css")
...
def rendered_targets(root: Path = ROOT) -> dict[Path, str]:
    ...
    for name in EXTRA_STYLESHEETS:
        ...
        index = replace_once(
            index,
            rf'href="/{re.escape(stem)}(?:\.[0-9a-f]{{{DIGEST_LEN}}})?\.css(?:\?v=[^"]+)?"',
            f'href="{hashed_url(static / name, root)}"',
            f"{name} reference",
        )
    sw = replace_once(
        sw,
    ...
    for name in EXTRA_STYLESHEETS:
        ...
        sw = replace_once(
            sw,
            rf'  "/{re.escape(stem)}(?:\.[0-9a-f]{{{DIGEST_LEN}}})?\.css",',
            f'  "{hashed_url(static / name, root)}",',
