#!/usr/bin/env python3
"""waf-bot：sidecar定期过目标站点的 JS 执行型 WAF，维护共享 cookie 供主服务复用。

雪球 2026-08-09 起对全部 API 启用接口拦截：裸 HTTP 客户端（httpx/curl_cffi）
一律 400（error_code 400016），必须执行挑战 JS 拿到 HttpOnly 共享 cookie 才能访问。
...
def refresh(target: dict) -> bool:
    """sidecar访问目标站点，等挑战自动执行完，把整套 cookie 写入共享文件。"""
