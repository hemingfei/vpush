...
- **早市**：7:00-8:00 之间随机开启，11:40-12:00 之间随机关闭；**午后**：12:30-12:50 随机开，16:00-16:30 随机关；**晚间**：19:00-19:30 随机开，23:30-23:55 随机关。窗口外零请求。
...
- **重启自动登录（2026-09-16）**：上一条的例外——工作日（周一至周五）08:00-22:00（北京时间，`restart_login_allowed`）内服务重启时自动补登一次，避免工作时段重启丢消息：重启正处的窗口重新武装（下个 tick 到点补登，会话持续到该窗口关窗）；落在窗口间隙时由 `_mx_restart_login_pending` 在窗口循环的第一个 tick 立即补登，后续由 4h 滚动兜底/晚间强关/下一窗口开窗收口。夜间与周末重启维持「错过不续连」旧口径。前提是 TOKEN 在 2 天时效内（`_mx_token_age_fresh`，与 2 天更换提醒同口径）：超龄 TOKEN 一律不自动登录，并发「系统通知」消息提醒管理员更换 TOKEN 后手动接入或等下个运行时段。
- **晚间兜底断开（2026-09-02，2026-09-03 起改为 23:30-23:55）**：23:30-23:55 之间随机一秒，若 MX 仍在线（如手动登录后忘断，无论会话来源是自动还是手动）一律强制断开（关标签页式 abort）——正常三段窗口最晚 23:55 前已关窗，这是防「人走了页面还开着」的最后保险，当天只执行一次。
