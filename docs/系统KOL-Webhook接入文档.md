...
- ⚠️ **token 会出现在服务器访问日志里**：地址在 URL 路径中，反代/uvicorn 的访问日志都会记录完整 URL（协议如此，无法避免）。这是「泄露走 regenerate 轮换」而不是「日志绝对保密」的原因；管理员侧应控制访问日志的留存范围。
...
- `timestamp` 传数字或字符串均可。
- 已知取舍：签名只覆盖 `timestamp`+密钥、**不含请求体**（飞书协议限制，保持兼容不改），
  因此不能防请求内容被篡改，防重放窗口也只有 ±1 小时；`token` 本身即凭据，签名是可选的第二道锁。
