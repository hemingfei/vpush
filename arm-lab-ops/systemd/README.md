...
CICC wrapper runs `scripts/cicc_arm_lab_sync.py --enable --limit N`. `N` comes from `$CACHE_ROOT/ops-lab-settings.json` `cicc_limit` when present, else `CICC_LAB_LIMIT` (default 3). `DRY_RUN=1` (unit default) uses `--dry-run`; `DRY_RUN=0` uses `--apply`. Logs: `$CACHE_ROOT/logs/cicc-lab-sync-*.log`. Cookie path is host `secrets/cicc-cookies.txt` — never commit.
